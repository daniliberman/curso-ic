...
public class DefinicionDeSteps {
	...
	public void noExisteElCliente(String nombreCliente) {
		Response respuesta =  get("http://10.10.35.6:8080/clients/v1.0/?name=" + nombreCliente);
		...
		if (respuesta.statusCode() == HttpStatus.SC_OK) {
			...
			delete("http://10.10.35.6:8080/clients/v1.0").
			then()
	...
	public void creoElCliente(String nombreCliente) {
		...
		given()
			...
			.contentType(ContentType.JSON).
		when()
			.post("http://10.10.35.6:8080/clients/v1.0").
		then()
			.statusCode(HttpStatus.SC_OK);	
	}
	...
	public void elListadoDeClientesIncluyeA(String nombreCliente) {
		when()
			.get("http://10.10.35.6:8080/clients/v1.0/?name=" + nombreCliente).
		then()
			.statusCode(HttpStatus.SC_OK)
